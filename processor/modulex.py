# last fetched on 2022-01-24 06:19:18.461297
# V2.0
##     ##  #######  ########  ##     ## ##       ######## ##     ##
###   ### ##     ## ##     ## ##     ## ##       ##        ##   ##
#### #### ##     ## ##     ## ##     ## ##       ##         ## ##
...
def require(modules_list: list[str]):
	modules_list = [modules_list] if type(modules_list) is not list else modules_list

	for m in modules_list:
		exec(f"global {m.replace('-','_')}")
		try:
			exec(f"import {m.replace('-','_')}")
